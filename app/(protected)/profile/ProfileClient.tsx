...
export default function ProfileClient({
  ...
  const [resumeFile, setResumeFile] = useState<string | null>(
    typeof window !== 'undefined' ? localStorage.getItem('pm_resume_name') : null
  )
  const [pmStory, setPmStory] = useState(
    typeof window !== 'undefined' ? localStorage.getItem('pm_story') ?? '' : ''
  )
  const [caseStudies, setCaseStudies] = useState<CaseStudy[]>(() => {
    if (typeof window === 'undefined') return []
    try {
      return JSON.parse(localStorage.getItem('pm_case_studies') ?? '[]')
    } catch {
      return []
    }
  })
  ...
  const publicSlug = profile.email.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '')
  ...
  const savePmStory = (value: string) => {
    setPmStory(value)
    localStorage.setItem('pm_story', value)
  }

  const addCaseStudy = () => {
    if (caseStudies.length >= 3) return
    const newStudies = [...caseStudies, { ...EMPTY_CASE_STUDY, id: Date.now().toString() }]
    setCaseStudies(newStudies)
    localStorage.setItem('pm_case_studies', JSON.stringify(newStudies))
  }
  ...
  const updateCaseStudy = (id: string, field: keyof CaseStudy, value: string) => {
    ...
    localStorage.setItem('pm_case_studies', JSON.stringify(updated))
  }
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(`https://pm-pathfinder.vercel.app/u/${publicSlug}`)
    setCopied(true)
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm transition-all ${
              activeTab === id
                ? 'bg-[#1e2d45] text-[#dae2fd] font-medium'
                : 'text-[#918fa1] hover:text-[#c7c4d8]'
            }`}
          >
          ...
          {activeTab === 'overview' && (
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-[#171f33] border border-white/[0.06] rounded-2xl p-6">
                <p className="text-[10px] uppercase tracking-widest text-[#918fa1] mb-4">
                  Profile Details
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={label}>
                      <p className="text-[10px] text-[#918fa1] uppercase tracking-widest mb-1">
                        {label}
                      </p>
              ...
              <div className="bg-[#222a3d] border border-white/10 rounded-2xl p-6 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-[#918fa1]">
                    Retake the diagnostic after completing a Learning Path milestone.
                  </p>
                </div>
          ...
          {activeTab === 'portfolio' && (
            <div className="flex flex-col gap-4">
              ...
              <BlurGate locked={true} label="Shareable portfolio link — Pro feature">
                <div className="bg-[#171f33] border border-white/[0.06] rounded-2xl p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-[#918fa1] mb-0.5">Your public portfolio link</p>
                      <p className="text-sm font-mono text-[#dae2fd] truncate">
                        pm-pathfinder.vercel.app/u/{publicSlug}
                      </p>
                    </div>
                    <button onClick={copyLink} className="flex items-center gap-1.5 text-xs text-teal-400 hover:text-teal-300 transition-colors">
              ...
              <div className="bg-[#171f33] border border-white/[0.06] rounded-2xl p-6">
                ...
                <textarea
                  value={pmStory}
                  onChange={(e) => savePmStory(e.target.value)}
                  placeholder="I bring 5 years of engineering experience to product management, with a track record of translating technical complexity into products users actually want. I'm currently transitioning into a PM role in fintech, focused on developer-facing products..."
                  rows={4}
                    ...
                    <div key={cs.id} className="bg-[#0f1729] border border-white/[0.06] rounded-xl p-4">
                      <div className="flex items-center justify-between mb-3">
                        <input
                          ...
                          placeholder="Project title (e.g. Redesigned onboarding for B2B SaaS)"
                      ...
                      {(['problem', 'approach', 'outcome'] as const).map((field) => (
                        <div key={field} className="mb-2.5 last:mb-0">
                          ...
                          <textarea
                            ...
                            placeholder={
                              field === 'problem'
                                ? 'What problem were you solving? Who was affected?'
                                : field === 'approach'
                                ? 'How did you approach it? What PM skills did you apply?'
              ...
              <div className="bg-[#171f33] border border-white/[0.06] rounded-2xl p-5">
                <p className="text-[10px] uppercase tracking-widest text-[#918fa1] mb-3">
                  Skills Card — auto-generated from your assessment
                </p>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[11px] text-[#918fa1]">Verified via PM Pathfinder assessment</span>
                </div>
                <ul className="flex flex-col gap-1.5">
          ...
          {activeTab === 'evaluations' && (
            <div className="flex flex-col gap-4">
              {evaluationHistory.map((eval_, i) => (
                <div
                  key={eval_.id}
                  className="bg-[#171f33] border border-white/[0.06] rounded-2xl p-5"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-sm font-medium text-[#dae2fd] capitalize">
                          {eval_.archetype}
                        </span>
                      </div>
                      <p className="text-xs text-[#918fa1]">
                    ...
                    {dimensions.map((dim) => {
                      ...
                      return (
                        <div key={dim} className="flex items-center gap-3">
                          ...
                          <span className="text-[11px] font-mono text-[#918fa1] w-8 text-right">
                            {score.toFixed(1)}
                          </span>
                          {delta !== null && (
                            <span
              ...
              <div className="bg-[#222a3d] border border-white/10 rounded-2xl p-5 text-center">
                <p className="text-sm font-medium text-[#dae2fd] mb-1">
                  Retake the diagnostic to track your progress
                </p>
                <p className="text-xs text-[#918fa1] mb-4">
                  Best done after completing a Learning Path milestone.
                </p>
                <Link href="/quiz/diagnostic">
