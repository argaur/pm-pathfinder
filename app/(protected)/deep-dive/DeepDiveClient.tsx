...
interface DeepDiveClientProps {
  initialTiers: Record<Dimension, string> | null
  initialCompleted: Dimension[]
}

export default function DeepDiveClient({ initialTiers, initialCompleted }: DeepDiveClientProps) {
  ...
  const [completedDimensions, setCompletedDimensions] = useState<Set<Dimension>>(new Set(initialCompleted))
  ...
  const handleAnswer = async (answer: string) => {
    ...
    if (currentQ < questions.length - 1) {
      ...
      try {
        ...
        setCompletedDimensions((prev) => new Set(prev).add(selectedDimension))
      } catch {
        // Non-blocking
      }
      setDone(true)
  ...
  if (done && selectedDimension) {
    ...
    const nextDim = DIMENSION_ORDER.find(
      (d) => d !== selectedDimension && !completedDimensions.has(d)
    )
