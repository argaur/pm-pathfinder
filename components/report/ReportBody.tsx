...
export default function ReportBody({
  ...
  return (
    <MotionConfig reducedMotion={REDUCED_MOTION}>
      {/* Archetype card */}
      <div className="bg-surface-1 rounded-2xl p-6 mb-6 border border-border">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-widest mb-1">Your PM Archetype</p>
          ...
          <div className="flex-shrink-0 text-xs font-mono text-muted-foreground bg-muted px-3 py-1.5 rounded-lg capitalize">
            {archetype.background.replace('_', '-')} × {archetype.mindset}
          </div>
        </div>

        <p className="text-sm text-foreground leading-relaxed mb-4">{archetype.description}</p>

        <div className="flex flex-wrap gap-2">
          {archetype.traits.map((t) => (
            <Badge
              key={t}
              className="bg-muted border border-border text-primary text-xs"
            >
              {t}
            </Badge>
          ))}
      ...
      <div className="mb-3 flex items-center justify-between">
        <p className="text-xs text-muted-foreground uppercase tracking-widest">Dimension Breakdown</p>
      ...
      <div className="bg-surface-1 rounded-xl p-5 border border-border">
        ...
        <div className="flex flex-wrap gap-2">
          {archetype.comesFrom.map((role) => (
            <span
              key={role}
              className="text-xs px-3 py-1.5 rounded-full bg-muted border border-border text-[#c7c4d8]"
            >
              {role}
            </span>
          ))}
